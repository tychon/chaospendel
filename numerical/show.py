...
class Pendulum(pygame.sprite.Sprite):
  ...
  def update(self):
    ...
    x2 = x1 + self.l/4*3 * math.sin(self.angles[1])
    y2 = y1 + self.l/4*3 * math.cos(self.angles[1])
...
def main():
  ...
  if time < 0 or opt_fps < 0 or time_step < 0:
    print "Invalid entries in "+project_name+".info"
    print "Values smaller 0 are not allowed."
    exit(1)
  if req_fps < 0:
